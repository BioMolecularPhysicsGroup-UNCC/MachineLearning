...
%               i-modes only get removed because an i-mode can be created.
%
...
%%                                                  error check input data
   ...
   if( n1 < 2 )
   error('n1 < 2: must have at least 2 representations of the 1-state');
   end
   if( n0 < 2 )
   error('n0 < 2: must have at least 2 representations of the 0-state');
   end
% -------------------------------
   if( nV1 ~= nV0 )
...
qType = gvSPLOC.qType;                        % type of clustering quality
...
%         before the rotation between a d-mode with a u-mode or an i-mode
...
% ---------------------------------------------------------- calculate icp
               ...
               icp3 = icp1 + icp2;                    % either one or both
...
% +++++++++++++++++++++++++++++++++++++++++++++++ calculate dQualityFactor
               vFraction = dVoteFraction10;
                ...
                %^^^^^-------------------------- only one needs to be good
                    if( (fImax > 0.5) && (dcp > 1) )   % qualify for bonus
...
% %    disp(['iVP_next = ',num2str( KEY( (j2Pair == j1_next) ) )]);
% %    disp('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^');
% %    disp('   ');
% %    pause
...
percSoft = round(1000*kCOUNT/tCOUNT)/10;
   if( verbosity > 0 )
   disp('   ');
   disp(['      cluster quality type = ',qType]);
   disp(['          [fDmode,fImode], =  [',num2str(fDm), ...
                                       ',',num2str(fIm),']']);
   disp(['      percent softGeometry = ',num2str(percSoft)]);
   disp(['              pursuit type = ',num2str(pursuitType)]);
   disp(msgInitial_U);
   disp(['            vote threshold = ',num2str(voteThreshold)]);
   disp([' discriminant subspace DIM = ',num2str(dd)]);
...
msg = ['----------------------------------------', ...
       '-------------------------- final status:']; 
fprintf(fid,'%s \n',msg);
msg = ['      percent softGeometry = ',qType];
fprintf(fid,'%s \n',msg);
msg = ['      cluster quality type = ',num2str(percSoft)];
fprintf(fid,'%s \n',msg);
msg = ['              pursuit type = ',num2str(pursuitType)];
fprintf(fid,'%s \n',msg);
fprintf(fid,'%s \n',msgInitial_U);
