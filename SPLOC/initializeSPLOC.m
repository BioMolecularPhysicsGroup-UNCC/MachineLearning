...
% 'qType'       {'hybrid','hardLinear','softGeometry'} => quality type
%                   Note: Different ways of defining clustering properties
%                   is possible, and can be specified by qType
%                   default = softGeometry
% 'wType'       {'fresh','append'} => start new file or append to previous
% 'bType'       [-1,1] use to bias the cluster quality factors.
%               fIm = 1 and fDm = 1 + bTypeValue       when bTypeValue < 0
%               fIm = 1 - bTypeValue and fDm = 1       when bTypeValue > 0
%               dQualityFactor -> fDm*dQualtityFactor
%               iQualityFactor -> fIm*iQualtityFactor
%               default value for bTypeValue = 0
% 'seed'        default (shuffle) but user can specify the seed.
%
...
%        initializeSPLOC(3,'fName','test','dType','pdfest')
%        initializeSPLOC(3,'fName','test','qType','hardLinear')
%        initializeSPLOC(3,'fName','test','qType','softGeometry')
%        initializeSPLOC(3,'fName','test','seed',12345)
% ------------------------------------------------------------------------
% PROCESS
...
% ----------------------------------------------------------- quality type
expectedQtype = {'hardLinear','softGeometry','hybrid'};
addParameter(p,'qType','hybrid', ...                             % default
             @(x) any(validatestring(x,expectedQtype)));
% ------------------------------------------------------------- write type
expectedWtype = {'fresh','append'};
addParameter(p,'wType','fresh', ...
             @(x) any(validatestring(x,expectedWtype)));
% ------------------------------------------------------------- bias level
biasValue = @(x) isnumeric(x) && isscalar(x) && (x >= -1) && (x <= 1);
addOptional(p,'bTypeValue',0,biasValue)       % # between between -1 and 1
% ------------------------------------------------------------------- seed
validSeed = @(x) isnumeric(x) && isscalar(x) && (x ~= 0);
addOptional(p,'seed0',0,validSeed)            % default prints out nothing
% the value of 0 is excluded because seed0 => use default. 
% ------------------------------------------------------------------------
parse(p,varargin{:});
...
qType = p.Results.qType;
wType = p.Results.wType;
bValue = p.Results.bTypeValue;
...
% ------------------------------------------------------- check bias level
   if( abs(bValue) < 0.0001 )    % set to default value of no special bias
   fIm = 1;                     
   fDm = 1;
   else
      if( bValue < 0 )   
...
msg = ['date = ',strDate];
fprintf(fid,'%s \n',msg);
msg = ['use pdf estimation = ',dType];
fprintf(fid,'%s \n',msg);
msg = ['clustering quality = ',qType];
fprintf(fid,'%s \n',msg);
...
msg = ['graphics file type = ',gFileType];
fprintf(fid,'%s \n',msg);
% ------------------------------------------- write information about seed
msg = dividerLine('seed input');
fprintf(fid,'%s \n',msg);
   if( seed0 == 0 )
   msg = 'seed selection = shuffle';
   else
   msg = ['seed selection = ',num2str(seed0)];
   end
fprintf(fid,'%s \n',msg);
